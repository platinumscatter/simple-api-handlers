...
type ServerInterface interface {
	...
	GetUserUserIdTasks(ctx echo.Context, userId uint) error
}
...
// GetUserUserIdTasks converts echo context to params.
func (w *ServerInterfaceWrapper) GetUserUserIdTasks(ctx echo.Context) error {
	var err error
	...
	// Invoke the callback with all the unmarshaled arguments
	err = w.Handler.GetUserUserIdTasks(ctx, userId)
	return err
}
...
func RegisterHandlersWithBaseURL(router EchoRouter, si ServerInterface, baseURL string) {
	...
	router.GET(baseURL+"/user/:user_id/tasks", wrapper.GetUserUserIdTasks)
...
type GetUserUserIdTasksRequestObject struct {
	UserId uint `json:"user_id"`
}

type GetUserUserIdTasksResponseObject interface {
	VisitGetUserUserIdTasksResponse(w http.ResponseWriter) error
}

type GetUserUserIdTasks200JSONResponse []Task

func (response GetUserUserIdTasks200JSONResponse) VisitGetUserUserIdTasksResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type StrictServerInterface interface {
	...
	// Get tasks for specific user
	// (GET /user/{user_id}/tasks)
	GetUserUserIdTasks(ctx context.Context, request GetUserUserIdTasksRequestObject) (GetUserUserIdTasksResponseObject, error)
}
...
// GetUserUserIdTasks operation middleware
func (sh *strictHandler) GetUserUserIdTasks(ctx echo.Context, userId uint) error {
	var request GetUserUserIdTasksRequestObject

	request.UserId = userId

	handler := func(ctx echo.Context, request interface{}) (interface{}, error) {
		return sh.ssi.GetUserUserIdTasks(ctx.Request().Context(), request.(GetUserUserIdTasksRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "GetUserUserIdTasks")
	}
	...
	if err != nil {
		return err
	} else if validResponse, ok := response.(GetUserUserIdTasksResponseObject); ok {
		return validResponse.VisitGetUserUserIdTasksResponse(ctx.Response())
	} else if response != nil {
		return fmt.Errorf("unexpected response type: %T", response)
	}
	return nil
